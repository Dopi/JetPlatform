...
public class ConfigTest extends AndroidTestCase {
    ...
    private class TotalConfig {
        ...
        public TotalConfig() {
            ...
            mMetrics.widthPixels = 480;
            mMetrics.heightPixels = 800;
            mMetrics.density = 1;
        }
    ...
    public void testAllConfigs() {
        ...
        config = new TotalConfig();
        config.setProperty(Properties.HEIGHT, 800);
        config.setProperty(Properties.WIDTH, 480);
        res = config.getResources();
        checkValue(res, R.configVarying.simple, "simple 800x480");
        checkValue(res, R.configVarying.bag,
                R.styleable.TestConfig, new String[]{"bag 800x480"});
