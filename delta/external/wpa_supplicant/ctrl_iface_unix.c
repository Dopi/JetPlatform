					    ...
					    socklen_t fromlen)
{
	...
	wpa_printf(MSG_ERROR, "CTRL_IFACE monitor attached");
	return 0;
}
					    ...
					    socklen_t fromlen)
{
	...
	while (dst) {
		if (fromlen == dst->addrlen &&
			...
			wpa_printf(MSG_ERROR, "CTRL_IFACE monitor detached");
			return 0;
		}
		prev = dst;
		dst = dst->next;
					      ...
					      void *sock_ctx)
{
	...
	wpa_printf(MSG_DEBUG,"recvfrom(ctrl_iface) cmd(%s)\n",buf);
	...
	if (os_strcmp(buf, "ATTACH") == 0) {
		if (wpa_supplicant_ctrl_iface_attach(priv, &from, fromlen))
			...
			new_attached = 1;
			wpa_printf(MSG_DEBUG,"new_attached == 1");
			reply_len = 2;
		}
	...
	if (reply) {
	    wpa_printf(MSG_DEBUG,"1. wpa_supplicant_ctrl_iface_receive(), sending reply %s\n",reply);
		sendto(sock, reply, reply_len, 0, (struct sockaddr *) &from,
		       fromlen);
		os_free(reply);
	} else if (reply_len == 1) {
	    wpa_printf(MSG_DEBUG,"2. wpa_supplicant_ctrl_iface_receive(), sending reply %s\n","FAIL");
		sendto(sock, "FAIL\n", 5, 0, (struct sockaddr *) &from,
		       fromlen);
	} else if (reply_len == 2) {
	    wpa_printf(MSG_DEBUG,"3. wpa_supplicant_ctrl_iface_receive(), sending reply %s\n","OK");
