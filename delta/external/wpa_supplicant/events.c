					 ...
					 struct wpa_scan_result *bss)
{
	...
	while ((ssid->proto & WPA_PROTO_WAPI) && bss->wapi_ie_len > 0) {
		...
		if ( ((ssid->wapi & 0x04) || (ssid->key_mgmt & WPA_KEY_MGMT_WAPI_PSK)) && (bss->wapi_ie[9]==2) ) {
			wpa_printf(MSG_DEBUG, "WAPI PSK network is selected based on WAPI IE");
			return 1;
		}

		if ( ((ssid->wapi & 0x08)  || (ssid->key_mgmt & WPA_KEY_MGMT_WAPI_CERT)) && (bss->wapi_ie[9]==1) ) {
	...
	if (proto_match == 0)
		wpa_printf(MSG_DEBUG, "   skip - no WAPI/WPA/RSN proto match");
			  ...
			  struct wpa_ssid **selected_ssid)
{
	...
	/* Second, try to find WPA-enabled AP */
				       ...
				       union wpa_event_data *data)
{
	...
	if ( (wpa_s->current_ssid) && (wpa_s->current_ssid->wapi) ) {
		...
		memcpy(own_s.v, wpa_s->wapi_own_addr, sizeof(own_s.v));
...
static void wpa_supplicant_event_disassoc(struct wpa_supplicant *wpa_s)
{
	...
	if ( (wpa_s->ap_wapi_ie_len>0) && (wpa_s->assoc_wapi_ie_len>0) ) { // this is a WAPI network
		...
		memset(wpa_s->ap_wapi_ie, 0, sizeof(wpa_s->ap_wapi_ie));
		memset(wpa_s->assoc_wapi_ie, 0, sizeof(wpa_s->assoc_wapi_ie));
...
wpa_supplicant_event_interface_status(struct wpa_supplicant *wpa_s,
				      union wpa_event_data *data)
{
	wpa_printf(MSG_DEBUG, "%s: wpa_s->ifname = %s\tdata->interface_status.ifname = %s\n", 
			   __FUNCTION__, wpa_s->ifname, data->interface_status.ifname);
			  ...
			  union wpa_event_data *data)
{
	switch (event) {
	case EVENT_ASSOC:
	    wpa_printf(MSG_DEBUG, "%s: case EVENT_ASSOC:\n", __FUNCTION__);
		wpa_supplicant_event_assoc(wpa_s, data);
		break;
	case EVENT_DISASSOC:
	    wpa_printf(MSG_DEBUG, "%s: case EVENT_DISASSOC:\n", __FUNCTION__);
