				...
				int line, const char *value)
{
	...
	wpa_printf(MSG_DEBUG, "%s: wpa_config_parse_string() returned %s\n", __FUNCTION__, tmp);
				...
				int line, const char *value)
{
	...
	*dst = atoi(value);
	wpa_printf(MSG_MSGDUMP, "wpa_config_parse_int(): %s=%d (0x%x)", data->name, *dst, *dst);
				...
				const char *value)
{
	if (*value == '"') {
		...
		wpa_printf(MSG_DEBUG, "%s: ssid->passphrase=%s\n", __FUNCTION__, ssid->passphrase);
		return 0;
	}
...
static char * wpa_config_write_psk(const struct parse_data *data,
				   struct wpa_ssid *ssid)
{
	/* to do: check wapi condition, both ascii and hex are in ascii format */
	if (ssid->passphrase)
	...
	while (*start != '\0') {
		...
		*end = '\0';
		if (os_strcmp(start, "WPA") == 0) {
			val |= WPA_PROTO_WPA;
			wpa_printf(MSG_DEBUG, "WPA network\n");
		}
				     ...
				     const char *value)
{
	...
	if (val == -1)
		return -1;
#ifdef WAPI
	if (val & ~(WPA_CIPHER_CCMP | WPA_CIPHER_TKIP | WPA_CIPHER_NONE | WPA_CIPHER_SMS4)) {
...
void wpa_config_free(struct wpa_config *config)
{
	...
	while (ssid) {
		prev = ssid;
		ssid = ssid->next;
		wpa_printf(MSG_DEBUG, "%s: freeing %s...", __FUNCTION__, prev->ssid);
	...
	os_free(config->pssid);
	os_free(config);
	wpa_printf(MSG_DEBUG, "Exit %s...", __FUNCTION__);
}
