...
#endif
	wpa_printf(MSG_DEBUG, "%s: State: %s -> %s", __FUNCTION__,
...
#endif
	wpa_printf(MSG_DEBUG, "wpa_supplicant_set_state(): wpa_s->wpa_state: %s", 
		   wpa_supplicant_state_txt(wpa_s->wpa_state));
}
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_DEBUG, "%s: ....1....\n", __FUNCTION__);
	if (wpa_s->confname == NULL)
		return -1;
	wpa_printf(MSG_DEBUG, "%s: ....2...\n", __FUNCTION__);
	conf = wpa_config_read(wpa_s->confname);
	wpa_printf(MSG_DEBUG, "%s: ....3...\n", __FUNCTION__);
	if (conf == NULL) {
	...
	wpa_printf(MSG_DEBUG, "%s: reconf_ctrl = %d\n", __FUNCTION__, reconf_ctrl);
	...
	wpa_printf(MSG_DEBUG, "%s: ....4...\n", __FUNCTION__);
	eapol_sm_invalidate_cached_session(wpa_s->eapol);
	wpa_s->current_ssid = NULL;
	wpa_printf(MSG_DEBUG, "%s: ....5...\n", __FUNCTION__);
	/*
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_PSK) {
		 ...
		 * authentication.
		 */
	    wpa_printf(MSG_DEBUG, "%s: ....6...\n", __FUNCTION__);
		eapol_sm_notify_eap_success(wpa_s->eapol, FALSE);
		wpa_printf(MSG_DEBUG, "%s: ....7...\n", __FUNCTION__);
	}
	eapol_sm_notify_config(wpa_s->eapol, NULL, NULL);
	wpa_printf(MSG_DEBUG, "%s: ....8...\n", __FUNCTION__);
	wpa_sm_set_config(wpa_s->wpa, NULL);
	wpa_printf(MSG_DEBUG, "%s: ....9...\n", __FUNCTION__);
	wpa_sm_set_fast_reauth(wpa_s->wpa, wpa_s->conf->fast_reauth);
	wpa_printf(MSG_DEBUG, "%s: ....10...\n", __FUNCTION__);
	rsn_preauth_deinit(wpa_s->wpa);
	wpa_printf(MSG_DEBUG, "%s: ....11...\n", __FUNCTION__);
	wpa_config_free(wpa_s->conf);
	wpa_printf(MSG_DEBUG, "%s: ....12...\n", __FUNCTION__);
	wpa_s->conf = conf;
	if (reconf_ctrl) {
	    wpa_printf(MSG_DEBUG, "%s: ....13...\n", __FUNCTION__);
		wpa_s->ctrl_iface = wpa_supplicant_ctrl_iface_init(wpa_s);
	    wpa_printf(MSG_DEBUG, "%s: ....14...\n", __FUNCTION__);
	}

	wpa_supplicant_clear_status(wpa_s);
	wpa_printf(MSG_DEBUG, "%s: ....15...\n", __FUNCTION__);
	wpa_s->reassociate = 1;
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_DEBUG, "Entering wpa_supplicant_scan()...\n");
	...
	if (scan_req != 2 && wpa_s->conf->ap_scan == 2) {
		...
		if (ssid == NULL) {
			...
			wpa_printf(MSG_DEBUG, "wpa_supplicant_scan: prev_scan_ssid==BROADCAST_SSID_SCAN");
			wpa_supplicant_req_scan(wpa_s, 0, 0);
			return;
		}
		if (ssid->next) {
	...
	wpa_printf(MSG_DEBUG, "%s: Starting AP scan (%s SSID)", __FUNCTION__,
		   ssid ? "specific": "broadcast");
	if (ssid) {
    		wpa_printf(MSG_DEBUG,"setting prev_scan_ssid to ssid %s", ssid->ssid);
		wpa_hexdump_ascii(MSG_DEBUG, "Scan SSID",
				  ssid->ssid, ssid->ssid_len);
		wpa_s->prev_scan_ssid = ssid;
	} else {
	    wpa_printf(MSG_DEBUG,"setting prev_scan_ssid to BROADCAST_SSID_SCAN");
		wpa_s->prev_scan_ssid = BROADCAST_SSID_SCAN;
	}
...
struct wapi_config * wapi_config_init(struct wpa_ssid *ssid)
{
	...
	if (config->wapi_policy & WAPI_KEY_MGMT_PSK) { /*bit 2:PSK*/
	    ...
	    wpa_printf(MSG_DEBUG, "PSK'%s'", ssid->psk);
	    wpa_printf(MSG_DEBUG, "passphrase'%s'", ssid->passphrase);
...
#endif
	wpa_s->current_ssid = ssid;
	wpa_printf(MSG_DEBUG, "%s: wpa_s->current_ssid->ssid=%s\n", 
		   __FUNCTION__, wpa_s->current_ssid->ssid);
#ifdef WAPI
	wpa_printf(MSG_DEBUG, "%s: wpa_s->current_ssid->wapi=%d\n", 
		   __FUNCTION__, wpa_s->current_ssid->wapi);
#endif
#ifdef WAPI
	if (ssid->wapi == 0) {
#endif
			       ...
			       int wait_for_interface)
{
	...
	wpa_printf(MSG_DEBUG, " wpa_s->l2 = %p ", wpa_s->l2);
	...
	if (wpa_s->l2 && l2_packet_get_own_addr(wpa_s->l2, wpa_s->own_addr)) {
		...
		wpa_printf(MSG_DEBUG, "Own MAC address: " MACSTR, MAC2STR(wpa_s->own_addr));
	...
	if (wpa_s->l2_wapi && l2_packet_get_own_addr(wpa_s->l2_wapi, wpa_s->wapi_own_addr)) {
		...
		wpa_printf(MSG_DEBUG, "Own WAPI MAC address: " MACSTR, MAC2STR(wpa_s->wapi_own_addr));
#endif
	...
	wpa_s->prev_scan_ssid = BROADCAST_SSID_SCAN;
	wpa_printf(MSG_DEBUG,"%s: wpa_supplicant_req_scan(wpa_s, %d, 100000)\n", 
		   __FUNCTION__, interface_count);
	wpa_supplicant_req_scan(wpa_s, 15, 100000);
