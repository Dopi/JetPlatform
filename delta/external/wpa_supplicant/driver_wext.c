...
wpa_driver_wext_event_wireless_custom(void *ctx, char *custom)
{
	...
	if (os_strncmp(custom, "MLME-MICHAELMICFAILURE.indication", 33) == 0) {
		...
		wpa_printf(MSG_DEBUG, "%s: Custom wireless event: 'ASSOCINFO(ReqIEs=", __FUNCTION__);
		spos = custom + 17;
...
int wpa_driver_wext_scan(void *priv, const u8 *ssid, size_t ssid_len)
{
	...
	if (ssid && ssid_len) {
#endif
	    wpa_printf(MSG_DEBUG,"wpa_driver_wext_scan(), scanning specific ssid %s\n", ssid);
		...
		wpa_printf(MSG_DEBUG,"wpa_driver_wext_scan(), scanning broadcast ssid\n");
...
#ifdef WAPI
			else if (clen > 8 && strncmp(custom, "wapi_ie=", 8) == 0 &&
                                   ap_num < max_size) {
				char *spos;
				int bytes;
				spos = custom + 8;
			  ...
			  struct wpa_driver_associate_params *params)
{
...
#ifdef WAPI
	else if (params->wpa_ie[0] == 0x44) //wapi
...
#ifdef WAPI
	if (params->wpa_ie[0] == 0x44) //wapi
		value |= IW_AUTH_CIPHER_SMS4;
#endif
	printf("params->pairwise_suite=%d, value = %d\n", params->pairwise_suite, value);
	...
	if (params->wpa_ie[0] == 0x44) //wapi
		value |= IW_AUTH_CIPHER_SMS4;
#endif
	printf("params->group_suite=%d, value = %d\n", params->group_suite, value);
	if (wpa_driver_wext_set_auth_param(drv,
	...
	if (params->wpa_ie[0] == 0x44)  { //wapi
		if (params->wpa_ie[9] == 2) //wapi psk
			...
			printf("unknown wapi key management value 0x%x\n", params->wpa_ie[9]);
	}
#endif

	printf("setting IW_AUTH_KEY_MGMT to 0x%x\n", value);
	if (wpa_driver_wext_set_auth_param(drv,
