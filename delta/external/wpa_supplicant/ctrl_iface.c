					    ...
					    char *buf, size_t buflen)
{
	char *pos, *end, tmp[30];
	int res, verbose, ret;
	wpa_printf(MSG_DEBUG, "Entering wpa_supplicant_ctrl_iface_status()... ");
	...
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	wpa_printf(MSG_DEBUG,"Entering wpa_supplicant_ctrl_iface_list_networks()...\n");
	pos = buf;
	end = buf + buflen;
	ret = os_snprintf(pos, end - pos,
	...
	while (ssid) {
    	...
    	wpa_printf(MSG_DEBUG,"Exiting wpa_supplicant_ctrl_iface_list_networks(), response length = %d\n",
		   pos-buf);
	return pos - buf;
}
	...
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	wpa_printf(MSG_DEBUG,"Entering wpa_supplicant_ctrl_iface_scan_results()...\n");
	...
	wpa_printf(MSG_DEBUG,"wpa_supplicant_ctrl_iface_scan_results():wpa_s->num_scan_results=%d\n",
	   wpa_s->num_scan_results);
	pos = buf;
	end = buf + buflen;
	...
	for (i = 0; i < wpa_s->num_scan_results; i++) {
		...
		wpa_printf(MSG_DEBUG,"wpa_s->scan_results[%d].ssid=%s\n", i, res->ssid);
...
//			    wpa_printf(MSG_DEBUG, "This is a WAPI PSK network");
	...
	//		    wpa_printf(MSG_DEBUG, "This is a WAPI CERT network");
    	...
    	wpa_printf(MSG_DEBUG,"Exiting wpa_supplicant_ctrl_iface_scan_results(), response length = %d\n",
		   pos-buf);
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (ssid == NULL) {
	    ...
	    wpa_printf(MSG_DEBUG, "%s: enabling network %s\n", __FUNCTION__, ssid->ssid);
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	/* to do: isn't the ssid has been freed at wpa_config_remove_network() already? */
	if (ssid == wpa_s->current_ssid) {
		/*
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*value++ = '\0';
	...
	wpa_printf(MSG_DEBUG,"%s: wpa_config_set() %s to %s\n", __FUNCTION__, name, value);
...
#ifdef WAPI
	if ((ssid->proto != WPA_PROTO_WAPI) && (ssid->wapi != 7) && (ssid->wapi != 11) && (ssid->key_mgmt != WPA_KEY_MGMT_WAPI_CERT)
		&& (ssid->key_mgmt != WPA_KEY_MGMT_WAPI_PSK) ) { 
	    /* only do eapol stuff if it's not a wapi network */
#endif
	...
	*name++ = '\0';
	...
	if (value == NULL) {
		...
		return -1;
    	    ...
    	    wpa_printf(MSG_DEBUG,"CTRL_IFACE, network variable %s has the value %s\n", name, value);
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (reply == NULL) {
	    wpa_printf(MSG_ERROR, "%s: os_malloc(%d) failed\n", __FUNCTION__, reply_size);
		*resp_len = 1;
		return NULL;
	}
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		wpa_printf(MSG_DEBUG,"wpa_supplicant_ctrl_iface_process(STATUS), reply_len=%d\n", 
			   reply_len);
	...
	*resp_len = reply_len;
	//wpa_printf(MSG_DEBUG,"wpa_supplicant_ctrl_iface_process(), *resp_len=%d, reply=%s\n",
		   //*resp_len, reply);
	return reply;
}
